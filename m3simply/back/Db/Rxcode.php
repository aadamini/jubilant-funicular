...
class Rxcode {
    ...
    public function search(){
       ...
       $result = $this->connessione->prepare("SELECT 
        t_creation_date.f_type as f_type,
        t_creation_date.f_category as f_category

        FROM `t_creation_date`

        WHERE t_creation_date.f_id = :id");
        $id = $this->needle;
        ...
        foreach($result as $riga){
            foreach ($riga as $key => $value){
                ...
                // manage errors with relative messages 
                if(count($result)!=1){
